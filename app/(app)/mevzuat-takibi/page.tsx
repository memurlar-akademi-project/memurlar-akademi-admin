...
const statusLabels: Record<string, string> = {
  ...
  awaiting_relevance_approval: "Kaynak doğrulaması",
  drafting: "Taslak hazırlanıyor",
  review_ready: "Revizyon onayı",
  partially_published: "Kısmen yayınlandı",
  completed: "Tamamlandı",
  no_changes_required: "Değişiklik gerekmiyor",
  rejected: "İlgisiz",
  failed: "Hata",
};

const registryIssueLabels: Record<string, string> = {
  instrument_number_missing: "Mevzuat numarası eksik",
  topic_article_ranges_missing: "Madde–konu aralığı eksik",
  registry_verification_required: "Resmî kayıt doğrulaması bekliyor",
  composite_subject: "Takip dışı · tek bir mevzuat değil",
};
      ...
      <section className="rounded-[22px] border border-[var(--color-admin-line)] bg-[var(--color-admin-panel)] p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            ["Kaynak doğrulaması", dashboard?.counts.awaiting_relevance_approval ?? 0],
        ...
        <div className="grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
          ...
          <section className="rounded-[22px] border border-[var(--color-admin-line)] bg-[var(--color-admin-panel)] p-5">
            {!selectedCase ? <p className="text-sm text-[var(--color-admin-muted)]">İncelenecek vaka yok.</p> : <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Info label="Mevzuat no" value={selectedCase.instrument?.instrument_number ?? "—"} /><Info label="Değişen madde" value={selectedCase.changed_articles?.join(", ") || "—"} /><Info label="Etkilenen sınav" value={selectedCase.affected_exam_names?.join(", ") || "—"} /><Info label="Etkilenen konu" value={String(selectedCase.affected_topic_ids?.length ?? 0)} /></div>
              ...
              {selectedCase.status === "awaiting_relevance_approval" ? <div className="flex flex-wrap gap-3"><button className="admin-button admin-button-primary" disabled={busyId !== null} onClick={() => void reviewCase("relevant")} type="button"><ShieldCheck size={17} /> AI analizini başlat</button><button className="admin-button admin-button-secondary" disabled={busyId !== null} onClick={() => void reviewCase("irrelevant")} type="button"><XCircle size={17} /> Yanlış eşleşme olarak kapat</button></div> : null}
